            ...
            <div class="container position-relative z-index-3 pb-5 h-100">
                <div class="row justify-content-center align-items-center pb-5 h-100">
                    <div class="col-lg-8 text-center pb-5 mb-5">
                        <h1 class="text-color-light font-weight-bold line-height-1 text-10 text-md-10 positive-ls-3 mb-3 appear-animation" data-appear-animation="blurIn" data-appear-animation-delay="1000" data-plugin-options="{'minWindowWidth': 0}">İSTANBUL KEPENK TAMİRİ</h1>
            ...
            <a href="{{ route('servicedetail', $item->slug)}}" title="İstanbul {{ $item->title}}" class="text-decoration-none">
                <div class="card custom-card-style-1 border-0 border-radius-0 custom-box-shadow-1">
                    <div class="card-body text-center px-4 py-5">
                        <h4 class="card-title alternative-font-4 text-color-dark font-weight-semibold line-height-1 text-3 mb-3 birsatir">{{ $item->title}}</h4>
                        <p class="font-weight-light text-color-dark line-height-7 mb-2 ikisatir">{{ $item->short}}</p>                    
                    </div>
                </div>
            </a>
        </div>
